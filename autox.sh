...
# install fping
# check curl fping is installed

if [[ -f /etc/redhat-release ]]; then
        release="centos"
        yum install curl fping -y
elif cat /etc/issue | grep -q -E -i "debian"; then
        release="debian"
        apt install curl fping -y
...
if test -s v2Dir/config.json; then
    ...
    echo "V2 directory does not exist! please modify!"
fi
function UrlTest() {
...
function ChangeIp() {
    ...
    if [[ $newIp != $v2Ip ]]; then
        ...
        echo $(date "+%Y-%m-%d %H:%M:%S")"    --新选IP与原IP一致，不更换" | tee -a /var/log/cfauto.log
    fi
}
#提供最佳测速10IP并保存到bestip.txt,并判断最高速的IP是否大于defaultSpeed的速度
function TestIp() {
    rm $workDir/top100.csv -rf
    echo $(date "+%Y-%m-%d %H:%M:%S")"    --对最优100 PING值进行测速，时间较长，稍等……" | tee -a /var/log/cfauto.log
    cat $workDir/top100ip.txt | while read line; do
...
function V2Test() {
    ...
    echo $(date "+%Y-%m-%d %H:%M:%S")"    --现有IP速度: "$nowSpeed "Mbps" | tee -a /var/log/cfauto.log
    #如果速度为0，可能是测速不正常，再次测速下
    if [[ $nowSpeed -lt 10 ]]; then
        sleep 10
        nowSpeed=$(UrlTest $v2Ip)
        echo $(date "+%Y-%m-%d %H:%M:%S")"    --二次测试完成: "$nowSpeed "Mbps" | tee -a /var/log/cfauto.log
    fi
    if [ $nowSpeed -ge $defaultSpeed ]; then
        exit 1
    fi
}
#判断原有优选过的IP存在,再次进行测试
function Best() {
    echo $(date "+%Y-%m-%d %H:%M:%S")"    --原有IP速度较慢，继续优选" | tee -a /var/log/cfauto.log
    if test -s $workDir/bestip.txt; then
        #  declare -i i=0
        rm $workDir/hot.csv -rf
        echo $(date "+%Y-%m-%d %H:%M:%S")"    --对BestIP进行测试，时间较长，稍等……" | tee -a /var/log/cfauto.log

        #while read line
        #do
        for ((i = 0; i < 10; i++)); do
            #statements
            a=$(awk '{print NR}' $workDir/bestip.txt | tail -n1)
            line=$(for ((i = 0; i < 1; i++)); do
                b=$((RANDOM % $a))
                b=$((b + 1))
                sed -n "$b"p $workDir/bestip.txt
            done)
            speed=$(UrlTest $line)
            echo $line','$speed >$workDir/hot.csv
            echo $(date "+%Y-%m-%d %H:%M:%S")"    --测试速度为: "$line"----"$speed "Mpbs" | tee -a /var/log/cfauto.log
            if [[ $speed -ge $defaultSpeed ]]; then
                echo $(date "+%Y-%m-%d %H:%M:%S")"    --找到高速的IP: "$line $speed "Mpbs" | tee -a /var/log/cfauto.log
                ChangeIp hot.csv
                exit 1
            fi
            #   [ $i -gt 0 ] && break
...
function Top() {
    ...
    if test -s $workDir/top100ip.txt; then
        TestIp
