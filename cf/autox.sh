...
function ChangeIp()
{
    ...
    if [[ $newIp != $v2Ip ]]; then
        echo $time'    --原IP：'$v2Ip '更换IP：'$newIp | tee  -a /var/log/cfauto.log
        sed -i s/$v2Ip/$newIp/g $v2Dir/config.json
        service v2ray restart
        echo $time'    --V2重启完成!' | tee  -a /var/log/cfauto.log
    else
        echo $time"    --优选IP与原IP一致，不需要更换"  | tee  -a /var/log/cfauto.log
    fi
}
...
function TestIp()
{
    ...
    echo $time"    --对最佳100 PING值进行测速，需要时间较长，请稍等" | tee  -a /var/log/cfauto.log
...
function V2Test(){  
    echo $time'    --对现V2IP: '$v2Ip '进行测试' | tee  -a /var/log/cfauto.log
    nowSpeed=` UrlTest $v2Ip  `
    echo $time"    --现有IP,速度为: "$nowSpeed "Mbps" | tee  -a /var/log/cfauto.log
    #如果速度为0，可能是测速不正常，再次测速下
    if [[ $nowSpeed -lt 10 ]]
    then
        nowSpeed=` UrlTest $v2Ip  `
        echo $time"    --二次测试完成,速度为: "$nowSpeed "Mbps" | tee  -a /var/log/cfauto.log
    fi
...
#判断原有优选过的IP存在,再次进行测试
function Best(){
    echo $time"    --原有IP速度较慢，进行下一步操作。" | tee  -a /var/log/cfauto.log
    if test -s $workDir/bestip.txt   
    then
      #  declare -i i=0 
        rm $workDir/hot.csv -rf
        echo $time"    --对优选BestIP进行测试，需要时间较长，请稍等!" | tee  -a /var/log/cfauto.log
        while read line
        do  
            speed=` UrlTest $line `
            ...
            then
                echo $time"    --找到defaultSpeedMpbs的IP和速度: "$line $speed "Mpbs"  | tee  -a /var/log/cfauto.log
                ChangeIp hot.csv 
                exit 1             
            fi
         #   [ $i -gt 0 ] && break
        ...
        #echo $time"    --优选IP测试完成,最佳IP和速度为: " `sort -n -r -k 2 -t , $workDir/hot.csv | sed -n "1, 1p" ` | tee  -a /var/log/cfauto.log
        #if [ $bestSpeed -ge $defaultSpeed ];then
        #    ChangeIp hot.csv
        #    exit 1
        # fi
        echo $time"    --优先IP速度太慢进行下一步测试。" | tee  -a /var/log/cfauto.log
    fi
}
function Top(){
#判断原有ping最小的100个ip是否存在,再次进行测试
    ...
    then
        TestIp
        echo $time"    --ping值top100IP测试完成！" | tee  -a /var/log/cfauto.log
        exit 1
    fi
}
...
function RandIp(){
    ...
    echo $time"    --IP生成完成，开始Ping测试，请稍等" | tee  -a /var/log/cfauto.log
    fping -f $workDir/ip.txt -c $num --interval=$interval -s >$workDir/ping.csv
    #fping包测试，先安装fping工具
    sed  -i '/\['` echo $num-1 | bc `'\]/! d' $workDir/ping.csv 
    #截取最后一次ping包的数据
    sed -i 's/\:.*.(/,/g;s/loss)//g;s/avg//g;s/[ ][ ]*//g'  $workDir/ping.csv
    #格式化ping文件，删除：至（之间的数据,删除loss)字符,删除avg,替换空格
    sort -n -k 3 -t , $workDir/ping.csv | sed -n '1,'$saveIp'p' | awk -F "," '{print $1}' >$workDir/top100ip.txt
